...
时间：mm:ss-mm:ss
画面：
[段内相对秒 如 0-3s] 一个能被单独生成的自足画面,依次写全:①场景(把【统一设定·场景】里该场景的地点+布景+光影原样写进来,别只写场景名) + ②出现的人物【连同 TA 的完整外貌一起写:年龄+发型发色+上衣颜色款式+下装+配饰,即把统一设定里那一行整句抄进这个画面,严禁只写"老师""女孩""小胖"这种光秃秃的代号】 + ③关键道具(把统一设定里的颜色材质原样写进来) + ④此刻的具体动作/表情 + ⑤景别/机位/运镜/光影风格
[3-7s] 下一个画面(同样把出现的人物外貌整句重复写进来)
（本段内切几个画面由这段解说内容动态决定;每个画面单独一行、以[相对秒]开头,相对时间连续、累计等于本段时长)
【✓正确示例】[0-3s] 昏暗辅导室,斑驳水泥墙+中央厚重木质砧板桌+顶部暖黄钨丝灯,一名中年男性(苍白皮肤深邃眼窝、黑色大背头、白色厨师服配深灰围裙、银丝边眼镜)面无表情地高举一把银色刀刃黑塑料柄的水果刀,低机位仰拍、电影感高反差硬光。
【✗错误示例】[0-3s] 辅导室,老师高举水果刀。(错在:只有代号"老师",没抄外貌 → Seedance 单独生成这画面时会给出一个完全不同长相的人,一致性崩掉)
音效：只写环境声/场景音 + 动作特效音(如脚步声、关门声、雨声、玻璃碎裂声等);【不要写背景音乐/BGM/配乐】。无则写 无
...
- 每个画面要具体、可视化、【完全自足】:Seedance 生成每个画面时【只能看到这一个画面的文字,看不到统一设定、也看不到别的画面】——所以每个画面里出现的人物,都必须把 TA 在统一设定里的完整外貌(年龄+发型发色+上衣颜色款式+下装+配饰)【整句抄进这个画面】,场景和关键道具同理。开头第一个画面给最有冲击力的钩子。
...
2. 【这是最常犯、也是你必须避免的错误】之后每一个画面里,凡出现某个人物/场景/道具,都要把它在统一设定里的那段外貌描述【一字不差地重复写进该画面】——严禁只用代号("老师""女孩""小胖""大壮"之类)一笔带过而不写外貌!因为单看一个画面时,"老师"两个字不含任何长相信息,Seedance 只会随机生成一个不相干的人,人物一致性就彻底毁了。宁可每个画面都啰嗦地重复外貌,也【绝不允许】出现光有代号、没有外貌描述的人物。场景、道具同理:每次出现都要带上它的固定外观。
3. 服装颜色款式、地点布景、道具外观必须与统一设定【逐字相同】,不许自己重新想象;同一地点在不同画面里不能一会儿这样一会儿那样。只有剧情明确发生改变(换衣服/换场景/道具损坏)才可变,并说明变成了什么;否则一律保持不变。
...
现在开始输出:先给【统一设定】锁定所有人物/场景/道具,再输出 ${n} 段(段内每段≥${minB}个画面、按内容动态切)。【务必记住:每个画面里凡出现人物,都要把该人物的完整外貌(年龄+发型+服装颜色款式+配饰)整句抄进这个画面,不能只写"老师""女孩"这种代号;场景和道具也要每次带上固定外观——这样每个画面单独喂给 Seedance 才能生成出同一个人。】`;
}
