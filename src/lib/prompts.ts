...
export function commentaryFormat(seg: number): string {
  return `
【输出格式 — 严格遵守,不要输出多余解释】
先输出一个总的【统一设定】块(把所有反复出现的人物/场景/道具的固定外观定死),再输出各段。统一设定块、各段之间都用单独一行 "----" 分隔。
关键:人物/场景/道具的外貌只写在【统一设定】和每段开头的【本段设定】里,【画面时间线里只用称谓引用、绝不重复外貌全文】。
...
场景：
· 场景名：地点+室内外+关键布景陈设+光影色调+氛围
道具：
· 道具名：外观颜色+材质+形状+显著特征（无关键道具可写 无）
----
第N段
时间：mm:ss-mm:ss
本段设定：（把"本段画面里会出现"的人物/场景/道具,从上面【统一设定】里【逐字照抄】列在这里,只列本段用到的;让整段自成一体、可整段喂给 Seedance）
· 人物 称谓：外貌(与统一设定逐字相同)
· 场景 场景名：布景光影(与统一设定逐字相同)
· 道具 道具名：外观(与统一设定逐字相同;无则不写这行)
画面：
[段内相对秒 如 0-3s] 只写:场景简称 + 出现的人物(用称谓,如"老师""女孩") + 此刻具体动作/表情 + 景别/机位/运镜/光影。【不要再重复外貌全文】
[3-7s] 下一个画面
...
时间：00:00-00:10
本段设定：
· 人物 老师：中年男性,苍白皮肤深邃眼窝,黑色大背头,白色厨师服配深灰围裙,银丝边眼镜
· 人物 女孩：16岁少女,白皙皮肤,齐肩黑色短发,藏蓝色校服百褶裙
· 场景 辅导室：昏暗室内,斑驳水泥墙,中央厚重木质砧板桌,顶部暖黄钨丝灯,阴冷压抑
· 道具 水果刀：银色锋利刀刃,黑色塑料柄
画面：
[0-3s] 辅导室,老师面无表情站在砧板桌后整理袖口,中景平视,电影感高反差硬光
[3-6s] 辅导室,女孩坐在桌前抱胸翻白眼一脸桀骜,近景微俯拍
[6-10s] 辅导室,老师双手将女孩死死按在砧板桌上、右手高举水果刀,女孩惊恐挣扎,低机位仰拍、强压迫感
音效：衣服摩擦声 + 沉闷的拍桌声 + 刀具挥动破空声

注意:尖括号只是示意,实际输出【不要带尖括号】,直接写内容。`;
}
...
- 画面时间线要【简洁】:每个 [秒数] 画面只写 场景简称 + 人物称谓(老师/女孩) + 具体动作表情 + 景别/机位/运镜/光影,【不要在画面里重复人物外貌全文】。开头第一个画面给最有冲击力的钩子。
【一致性做法 — 外貌只在"设定"里写,时间线里不写】同一【人物/场景/道具】全片必须长得一模一样,靠下面两处"设定"锁定,而不是靠在每个画面里重复:
1. 开头的【统一设定】:把每个反复出现的人物(服装颜色款式/发型/年龄)、场景(布景陈设/光影色调)、关键道具(颜色/材质/形状)彻底定死,全片只写这一次总表。
2. 每段开头的【本段设定】:把"这一段里会出现"的人物/场景/道具,从统一设定里【逐字照抄】列一遍(只列本段用到的),让整段自成一体、可整段喂给 Seedance。
3. 画面时间线里【只用称谓引用】(老师/女孩/水果刀),绝不重复外貌全文;但动作和场景描述不能与设定冲突。
4. 服装颜色款式/地点布景/道具外观,在【统一设定】和各段【本段设定】里必须逐字相同,不许自己重新想象;只有剧情明确改变(换衣/换场景/道具损坏)才可变,并说明变成了什么。
- 只输出 统一设定 / 各段(本段设定+时间+画面+音效),不要配音、不要旁白/解说文字、不要台词。${styleBlock(style)}
...
现在开始输出:先给总的【统一设定】锁定所有人物/场景/道具,再输出 ${n} 段(段内每段≥${minB}个画面、按内容动态切)。【务必记住:每段开头写【本段设定】,把本段出现的人物/场景/道具的固定外观从统一设定里逐字照抄列一次;画面时间线里只用称谓(老师/女孩)引用,不要重复外貌全文——外貌只在设定里写,时间线保持简洁。】`;
}
